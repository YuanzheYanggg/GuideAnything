...
- 业务阶段是画布的一级结构，按作者定义顺序从上到下显示，支持命名与排序；可选责任泳道从左到右显示，混合支持角色与系统，主流程节点可归属“阶段 × 泳道”。一级主流程仅包含无来源追踪的开始、结束、流程、判断、数据和子指南节点。旧画布未设置阶段或泳道时仍可编辑、发布并保持原布局。
...
- “自动整理”按入口和主流程连接关系计算布局：含泳道时按“阶段行 × 责任列”放置主流程，无泳道时保留既有从左到右布局；资料放入所属步骤详情轨道，并给出阶段、泳道、未归类资料、孤立流程和循环的预览诊断。预览不保存、不进入撤销历史；作者明确应用后才写入草稿，且可撤销。
