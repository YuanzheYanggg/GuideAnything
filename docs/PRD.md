...
GuideAnything 是面向 ERP 培训场景的多用户、多模态在线教学工具。作者在无限画布中组合流程节点、Markdown、图片、视频关键点和子指南，发布后由学习者检索并按步骤浏览。产品只实现内部发布，不包含公网部署、付费资源或外部通知。
...
1. 作者登录并创建 ERP 教学指南。
2. 在无限画布添加流程图形、Markdown、图片、视频节点和视频关键点，建立方向连线与教学步骤。
...
- 保存 `nodes + edges + viewport + steps`。编辑器只订阅必要状态，节点组件记忆化；折叠内容使用 `hidden`，图片与视频使用懒加载。
...
- 学习者从搜索结果进入只读播放页，使用上一步/下一步导航；画布聚焦当前节点，视频关键点可跳转。
