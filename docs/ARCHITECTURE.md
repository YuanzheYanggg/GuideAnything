...
1. 若已存在同 `referenceNodeId` 的展开产物，原样返回，保证幂等。
2. 计算源图包围盒，将源入口节点平移到引用节点右侧 160px，并将所有坐标按同一向量转换。
3. 为所有源节点和边创建确定性命名空间 ID；重写边端点、步骤关联和视频关键点目标。
4. 将指向引用节点输入端的边保留；新增引用节点到源入口的代理边；源出口到引用节点输出端的后续边继续由引用节点承接。
5. 产物携带来源元数据。折叠仅设置这些节点和边的 `hidden=true`；再次展开恢复为 `false`，不重新复制。
