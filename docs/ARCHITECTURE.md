...
路由负责解析和响应，repository 隔离 SQL，service 组合身份、权限和事务。授权必须在 API 完成，工作区权限与指南级授权是两个独立边界：

- 工作区 `OWNER` 可管理工作区设置、成员和资源生命周期；工作区 `EDIT` 允许具备 `AUTHOR/EDITOR` 全局角色的用户在该工作区创建指南；`VIEW` 只提供基础工作区可见性。这些权限都不自动授予对他人指南草稿的内容编辑或发布权。
- 指南 owner 可读写草稿、管理协作者并发布；显式指南 collaborator 可读写该草稿，但不能发布或管理协作者。工作区 `OWNER` 如果不是该指南 owner/collaborator，仍不自动获得指南内容编辑或发布权。
- 显式指南 collaborator 进入“与我共享”；普通工作区成员不会被误认为显式共享。
- 收藏和最近查看属于当前用户；写入前重新验证资源可见性。
- 当请求者没有基础可见性、需要隐藏工作区/资源是否存在时返回 404；当对象已可见/可读，但请求者缺少所请的创建、编辑、发布、设置或生命周期权限时返回 403。前端按钮隐藏不是安全边界。
- 回收使用 `workspace_items.deleted_at/deleted_by`；永久移除只允许指南所有者或工作区所有者。

## 5. 画布与固定版本引用

`CanvasDocument` 保存节点、连线、视口、步骤和入口/出口。`canvas-core` 从文档派生阶段优先布局、正交避障路由和图片标注镜头，不把展示路由写回协议。连续资料链参与连接排名；显式 `contentParentId` 资料仍按挂靠层级放置。子指南引用固定到不可变 `guide_versions` 快照；展开使用确定性命名空间、来源追踪和桥接边，折叠恢复原续接边。资源被回收或已发布指南被永久移除时，不删除历史版本，因而不破坏已存引用。

图片标注作为图片节点可选数据随草稿和不可变版本保存。编辑器只写入归一化区域、文案、顺序、镜头和目标节点 ID；学习页用媒体预览栈解析目标并保留返回时的标注索引，不创建第二套教学步骤或业务边。
