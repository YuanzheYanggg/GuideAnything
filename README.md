...
GuideAnything 是一个前后端分离的多用户、多模态 ERP 在线教学工具。作者先梳理业务主流程，再把 Markdown、图片和视频资料挂靠到对应步骤；学习者从资料库检索已发布指南，并按业务阶段、步骤和资料逐步学习。
...
2. 先从顶部工具栏添加开始、流程、判断、数据或子指南，拖动节点边缘的输入/输出端口把业务主流程连接起来。
3. 在“流程结构”面板新增并排序业务阶段；选中一个主流程节点后再新增 Markdown、图片或视频，资料会自动挂靠到该步骤。也可在右侧属性面板改挂靠目标，或将资料脱离为未归类资料。
4. 从左侧“流程结构”检查“阶段 → 主流程 → 资料”的层级，并优先处理面板标出的未归类资料。
5. 使用“自动整理”先生成预览：确认阶段泳道、主流程顺序和资料详情轨道后选择“应用”；不符合预期时选择“取消”，取消不会写入草稿或历史记录。应用后仍可用撤销恢复。
6. 使用多选、复制/粘贴、左对齐、置顶/置底、删除、撤销/重做继续编辑；选中节点后可“加入教学步骤”。
7. 草稿在修改停止 1.5 秒后自动保存，也可 `Ctrl/Cmd+S` 显式保存。服务端用 `revision` 乐观锁拒绝静默覆盖。
8. 发布后产生递增的不可变版本。
...
3. 当前步骤会聚焦流程节点，并汇总展示该步骤挂靠的 Markdown、图片和视频资料；视频关键点按钮会跳转到对应时间。
