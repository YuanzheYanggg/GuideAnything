...
7. 使用“自动整理”先生成预览：含泳道时确认“阶段行 × 责任列”、主流程顺序和资料详情轨道；未配置泳道的旧文档保持原布局。选择“应用”后写入草稿；选择“取消”不会写入草稿或历史记录，应用后仍可用撤销恢复。
...
3. 当前步骤会聚焦流程节点，显示其“责任 / 系统”提示，并汇总展示该步骤挂靠的 Markdown、图片和视频资料；视频关键点按钮会跳转到对应时间。未配置泳道的旧版本不会显示责任提示；展开子指南内容仅展示引用节点的责任语境。
