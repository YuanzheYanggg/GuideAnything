...
export const ImageNode = memo(function ImageNode({ id, data, selected, width, height }: NodeProps) {
  ...
  return <NodeChrome selected={selected} tone="image" width={width} height={height}><span className="node-kicker">IMAGE</span>{source ? <img src={source} alt={value.alt} loading="lazy" /> : <p>图片载入失败</p>}<InlineNodeTextEditor nodeId={id} field="imageCaption" value={value.caption ?? ''} label={`${value.alt} · 图片说明`} multiline placeholder="双击添加图片说明" showPlaceholder={Boolean(selected)}>{value.caption ? <p>{value.caption}</p> : null}</InlineNodeTextEditor>{summary ? <small className="image-annotation-summary">{summary}</small> : null}</NodeChrome>;
});
