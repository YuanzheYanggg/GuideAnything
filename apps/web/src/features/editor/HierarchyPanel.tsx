  ...
  return <aside className="hierarchy-panel" aria-label="流程结构">
    ...
    <section className="hierarchy-manager" aria-label="业务阶段管理">
      ...
      {stages.map((stage, index) => <div className="hierarchy-manager-row" key={stage.id}>
        <input aria-label={`业务阶段 ${stage.title}`} value={stageDrafts[stage.id] ?? stage.title} disabled={editingLocked} onChange={(event) => updateStageTitle(stage.id, event.target.value)} onBlur={() => clearStageDraft(stage.id)} />
        <div className="hierarchy-manager-actions"><button type="button" aria-label={`上移阶段 ${stage.title}`} onClick={() => onMoveStage(stage.id, -1)} disabled={editingLocked || index === 0}>↑</button><button type="button" aria-label={`下移阶段 ${stage.title}`} onClick={() => onMoveStage(stage.id, 1)} disabled={editingLocked || index === stages.length - 1}>↓</button><button type="button" aria-label={`删除阶段 ${stage.title}`} onClick={() => onRequestDeleteStage(stage.id)} disabled={editingLocked}>×</button></div>
      </div>)}
    ...
    <section className="hierarchy-manager" aria-label="责任泳道管理">
      ...
      {lanes.map((lane, index) => <div className="hierarchy-manager-row" key={lane.id}>
        <input aria-label={`责任泳道 ${lane.title}`} value={laneDrafts[lane.id] ?? lane.title} disabled={editingLocked} onChange={(event) => updateLaneTitle(lane.id, event.target.value)} onBlur={() => clearLaneDraft(lane.id)} />
        <span className={`lane-kind-badge lane-kind-${lane.kind.toLowerCase()}`}>{lane.kind === 'ROLE' ? '角色' : '系统'}</span>
        <div className="hierarchy-manager-actions"><button type="button" aria-label={`上移泳道 ${lane.title}`} onClick={() => onMoveLane(lane.id, -1)} disabled={editingLocked || index === 0}>↑</button><button type="button" aria-label={`下移泳道 ${lane.title}`} onClick={() => onMoveLane(lane.id, 1)} disabled={editingLocked || index === lanes.length - 1}>↓</button><button type="button" aria-label={`删除泳道 ${lane.title}`} onClick={() => onRequestDeleteLane(lane.id)} disabled={editingLocked}>×</button></div>
      </div>)}
